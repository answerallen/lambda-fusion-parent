 ...
 * 支持租户数据源的命名、创建、删除、查询和编程式切换。
 * </p>
 * 
 * <p>使用示例：</p>
 * <pre>
 * // 获取租户数据源名称
 * String dsName = tenantDataSourceManager.getTenantDataSourceName("1001", "ai-tenant-");
 * 
 * // 编程式切换到租户数据源
 * try (DataSourceSwitcher switcher = 
 *         tenantDataSourceManager.switchToTenantDataSource("1001", "ai-tenant-")) {
 *     // 执行租户数据库操作
 * }
 * </pre>
 */
...
public class TenantDataSourceManager {
    ...
    public boolean createTenantDataSource(String tenantId, String tenantPrefix, RemoteDataSource dataSourceConfig) {
        ...
        if (success) {
            log.info("Created tenant datasource: {} for tenant: {}", dataSourceName, tenantId);
            String cacheKey = tenantPrefix + ":" + tenantId;
    ...
    public boolean deleteTenantDataSource(String tenantId, String tenantPrefix) {
        ...
        if (success) {
            log.info("Deleted tenant datasource: {} for tenant: {}", dataSourceName, tenantId);
            String cacheKey = tenantPrefix + ":" + tenantId;
    ...
    public void clearCache() {
        ...
        log.info("Cleared tenant datasource cache");
    }
