package com.lamuda.cloud.scaffold.dict.controller;

import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.google.common.collect.Maps;
import com.lamuda.cloud.core.utils.OperatorUtils;
import com.lamuda.cloud.fx.core.base.user.LoginUserDetails;
import com.lamuda.cloud.fx.dict.entity.DictInfo;
import com.lamuda.cloud.fx.dict.entity.DictInfoVO;
import com.lamuda.cloud.fx.dict.entity.DictType;
import com.lamuda.cloud.scaffold.dict.service.DictInfoService;
import com.lamuda.cloud.scaffold.dict.service.DictTypeService;
import com.lamuda.cloud.scaffold.dict.support.enums.DictContextHolders;
import com.lamuda.cloud.scaffold.dict.support.enums.DictHolder;
import com.lamuda.cloud.fx.dict.dto.QueryDictTree;
import com.lamuda.cloud.fx.dict.vo.SimpleDictTypeVo;
import com.lamuda.cloud.logger.annotation.OperationLog;
...
import static com.lamuda.cloud.fx.core.utils.ParameterUtils.fuzzyQuery;
...
public class DictController {
    ...
    public List<DictType> getDictTypeList(@Parameter(description = "字典类型") @RequestParam(required = false) String type) {
    ...
    public Map<String, SimpleDictTypeVo> getAllDynamicDictInfo(@Parameter(description = "字典类型") @RequestParam(required = false) String type) {
    ...
    public DictType saveDictType(@Valid @Parameter(description = "字典类型数据", required = true) @RequestBody DictType dictType) {
    ...
    public List<DictInfo> queryDictInfoByParentId(@Parameter(description = "数据项父ID", required = true) @PathVariable String parentid) {
        return dictInfoService.queryDictInfoByParentId(parentid);
    }

    @OperationLog
    @PutMapping("/dicttype")
    @Operation(summary = "更新字典类型", description = "更新字典类型", parameters = {
    ...
    @Operation(summary = "字典类型分页查询", description = "字典类型分页查询", parameters = {
    ...
    public Page<DictType> dictTypeList(@PathVariable(required = false) Integer number,
                                       @PathVariable(required = false) Integer size, String dictName) {
    ...
    @Operation(summary = "数据项条件查询", description = "分页查询所有数据列表", parameters = {
            @Parameter(name = "dictType", description = "字典类型", in = ParameterIn.QUERY),
            @Parameter(name = "fieldType", description = "字段类型", in = ParameterIn.QUERY),
            @Parameter(name = "dictInfoId", description = "数据项Id", in = ParameterIn.QUERY),
            @Parameter(name = "fieldName", description = "字段名称", in = ParameterIn.QUERY),
    ...
    public List<DictInfo> selectDictInfo(@RequestParam(required = false) String dictType,
                                         @RequestParam(required = false) String fieldType,
                                         @RequestParam(required = false) String dictInfoId,
                                         @RequestParam(required = false) String fieldName,
                                         @RequestParam(required = false) Integer enableState
    ...
    @OperationLog
    @PutMapping("/dictinfo")
    @Operation(summary = "更新字典详细信息", description = "更新字典详细信息", parameters = {
            @Parameter(name = "id", description = "id", required = true, in = ParameterIn.QUERY),
            ...
            @Parameter(name = "enableState", description = "启用状态：0未启用，1启用", required = true, in = ParameterIn.QUERY, schema = @Schema(allowableValues = {"0", "1"})),
            @Parameter(name = "remarks", description = "备注", in = ParameterIn.QUERY),
            @Parameter(name = "additional", description = "额外参数")
    })
    public DictInfo updateDictInfo(@Valid DictInfoVO dictInfoVO, @RequestBody(required = false) DictInfo.Additional additional) {
