package com.lamuda.cloud.scaffold.dict.service.impl;
...
import com.lamuda.cloud.core.utils.FastCopyUtils;
import com.lamuda.cloud.core.utils.OperatorUtils;
import com.lamuda.cloud.fx.core.base.service.BaseServiceImpl;
import com.lamuda.cloud.fx.core.base.user.LoginUserDetails;
import com.lamuda.cloud.fx.core.tree.TreeFactory;
import com.lamuda.cloud.fx.core.utils.ParameterUtils;
...
import com.lamuda.cloud.scaffold.dict.mapper.DictInfoMapper;
import com.lamuda.cloud.scaffold.dict.mapper.DictTypeMapper;
...
import static com.lamuda.cloud.fx.core.Constants.JOINER;
...
public class DictInfoServiceImpl extends BaseServiceImpl<DictInfo, DictInfoVO, DictInfoMapper> implements DictInfoService, IService<DictInfo> {
    ...
    public DictInfo saveDictInfo(LoginUser operator, DictInfo source) {
        ...
        source.setExtra(CollectionUtils.isNotEmpty(source.getParameters()) ? convertJson(source.getParameters()) :
                null);
    ...
    public DictInfo updateDictInfo(DictInfo dictInfo) {
        ...
        dictInfo.setExtra(CollectionUtils.isNotEmpty(dictInfo.getParameters()) ?
                convertJson(dictInfo.getParameters()) : null);
    ...
    public Map<String, Object> getStaticDictInfoGroup(String type) {
        String tenantId = ((LoginUserDetails) OperatorUtils.getOperator()).getTenantId();
        List<DictInfoGroup> lists = dictInfoMapper.getAllDictInfoGroup(StringUtils.isNotBlank(type) ? ParameterUtils.fuzzyQuery(type) : type, tenantId);
    ...
    @Override
    public Map<String, SimpleDictTypeVo> getDynamicDictInfoGroup(String type) {
        ...
        Map<String, SimpleDictTypeVo> result = Maps.newHashMapWithExpectedSize(dictTypes.size());
        ...
        for (DictType dictType : dictTypes) {
            final SimpleDictTypeVo vo = FastCopyUtils.copy(dictType, SimpleDictTypeVo.class);
        ...
        for (DictInfoGroup info : lists) {
            if (result.containsKey(info.getDictType())) {
                final SimpleDictTypeVo vo = result.get(info.getDictType());
    ...
    public List<DictInfo> subTreeData(String type) {
        ...
        if (StringUtils.isNotBlank(type)) {
            ...
            if (CollectionUtils.isNotEmpty(dictTypes)) {
                ...
                list.forEach(info -> info.setParameters(StringUtils.isNotBlank(info.getExtra()) ? convertMap(info.getExtra()) : null));
