...
import com.lamuda.cloud.fx.core.tree.ITreeDataFilter;
...
import com.lamuda.cloud.scaffold.dict.support.resolve.IDynamicDictResolve;
...
public class DictTypeServiceImpl extends ServiceImpl<DictTypeMapper, DictType> implements DictTypeService {
    ...
    public boolean dictTypeExists(DictType dictType) {
        ...
        wrapper.eq(DictType::getDictType, dictType.getDictType()).or().eq(DictType::getDictName, dictType.getDictName());
    ...
    public List<DictType> dynamicTreeList(QueryDictTree queryDictTree) {
        ...
        if (dataType == null || dataType == DATA_TYPE_ENUM) {
            // 枚举字典
            if (StringUtils.isNotBlank(type)) {
                DictType conditions = dictTypeMapper.selectOne(new LambdaQueryWrapper<DictType>().eq(DictType::getDictType, type));
                ...
                if (CollectionUtils.isNotEmpty(enumList)) {
                    if (StringUtils.isNotBlank(name)) {
                        List<DictType> list = enumList.stream().filter(enumDict -> StringUtils.contains(enumDict.getDictName(), name)).collect(Collectors.toList());
                        result.addAll(list);
                    } else {
        ...
        final List<DictType> typeList = treeDataFilter.filter(result, queryDictTree.getType(), DictType::getDictType, DictType::getId, DictType::getParentKeys,
                target -> target.stream().sorted(Comparator.comparing(DictType::getDictName)).collect(Collectors.toList()));
    ...
    public void deleteDictType(String id) {
        if (!dictionaryProperties.isAllowedCascadeDelete()) {
            List<DictType> types = dictTypeMapper.selectList(Wrappers.lambdaQuery(DictType.class)
        ...
        List<DictType> cascadeDictType = dictTypeMapper.selectList(new LambdaQueryWrapper<DictType>()
                .eq(StrUtil.isNotEmpty(id),DictType::getId, id)
        );
                ...
                if (dynamicDictResolve.isSupport(dictType.getDataType())) {
                    ...
                    if (CollectionUtils.isNotEmpty(list)) {
                        list.forEach(v -> {
                            ...
                            dictInfo.setId(StringUtils.isNotBlank(v.getId()) ? v.getId() : v.getVal().toString());
                    ...
                    if (hasRank.get()) {
                        // 有级别
                        dictInfoList = TreeFactory.build3(dictInfos, DictInfo::getId, DictInfo::getParentId, DictInfo::level, DictInfo::setChildren);
                    } else {
                        // 无级别
                        dictInfoList = TreeFactory.build2(dictInfos, DictInfo::getId, DictInfo::getParentId, DictInfo::setChildren);
                    }
