...
import static com.lamuda.cloud.scaffold.core.tree.DragMode.BEFORE;
import static com.lamuda.cloud.scaffold.core.tree.DragMode.CHILD;
...
public final class TreeUtils {
    ...
    public static <T extends Tree<T>> List<T> getAllChangedAfterMoved(@Nonnull T resource, @Nonnull T target, DragMode mode,
        ...
        Optional<T> optional1 = children.stream().filter(item -> item.id().equals(id1)).findFirst();
        Optional<T> optional2 = children.stream().filter(item -> item.id().equals(id2)).findFirst();
     ...
     */
    private static <T extends Tree<T>> List<T> childrenHandler(String searchString, String replacement, List<T> children2) {
