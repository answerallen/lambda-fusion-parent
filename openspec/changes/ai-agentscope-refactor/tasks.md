...
- [x] AppEntity 字段扩展 阶段 B（2026-07-18）：**toolIds 消费**--`ToolToolkitAdapter.buildToolkit(toolIds)` 按 @Tool name 过滤（null/空=全部，`hasMatchingTool` 按 `Tool.name()`/方法名匹配，bean 级过滤）；**mcpServerIds 消费**--`AgentRuntimeServiceImpl.buildToolkit` 按 mcpServerIds for-each `mcpClientAdapter.get(id)` -> `toolkit.registerMcpClient(...).block()`（per-app，单 server 失败跳过不阻断）；`AgentTemplate` 加 `toolIds`/`mcpServerIds`（从 app 实时读，不入快照，不 override）。阶段 D（toolGroups + middlewareConfig）待做。
- [x] AppEntity 字段扩展 阶段 C（2026-07-18）：**subagentSpec 消费**--新建 `SubagentSpecDto`（record：name/toolName/description/sysPrompt/modelId/temperature/maxTokens/kbIds/ragMode/toolIds/mcpServerIds）+ `SubagentSpecParser`（`@Component`，Jackson 3.x `tools.jackson.databind.ObjectMapper` 解析 JSON 数组，失败返空列表不阻断）；`AgentTemplate` 加 `subagentSpec`（从 app 实时读，不入快照，不 override）；`AgentRuntimeServiceImpl.buildAgent` 提前解析 `stateStore`（stateful 时 `agentStateStoreProvider.getIfAvailable()`）后调 `registerSubagents(toolkit, t, stateStore)`：每个 spec 构造子 `AgentTemplate`（tenantId/retrievalTopK 继承父 agent，subagentSpec=null **一层不嵌套避免递归循环**）-> `SubAgentProvider<HarnessAgent> (() -> buildAgent(child, false))`（每次工具调用产出新实例，复用 buildAgent）+ `SubAgentConfig.builder().toolName(可选)/description/stateStore(主 agent store，null 走默认 InMemory).build()` -> `new SubAgentTool(provider, config)` -> `toolkit.registerAgentTool(...)`（子 agent 复用主 stateStore 会话隔离，各自 model/toolkit 按 spec）。`templateFromApp`/`templateFromSession` 读 `app.getSubagentSpec()`，`mergeOverride` 透传。API 依据 sources jar 核实：`SubAgentTool(SubAgentProvider<?>, SubAgentConfig)` / `SubAgentProvider<T extends Agent>.provide()` / `SubAgentConfig.builder().toolName/description/forwardEvents/streamOptions/stateStore`（编译绿，印证 `HarnessAgent` 是 `Agent` 子类型）。
- [x] AppEntity 字段扩展 阶段 D（2026-07-18）：**toolGroups 消费**--新建 `ToolGroupSpecDto`（record：name/description/active/toolNames）+ `ToolGroupSpecParser`；`ToolToolkitAdapter` 拆 `registerTools(Toolkit, toolIds, bindings)`（注册到外部 toolkit，有组 `registration().tool(bean).group(name).apply()`，无组 `registerTool(bean)`，bean 级分组）；`AgentRuntimeServiceImpl.buildToolkit` 改 `new Toolkit()` + `registerToolGroups`（解析 spec -> `createToolGroup(name, desc, active)` + 产出 toolName->组名 bindings）+ `registerTools`（组须先 createToolGroup，注册时 `validateGroupExists` 校验）。**middlewareConfig 消费**--新建 `MiddlewareConfigDto`（record：type/enabled/params）+ `MiddlewareConfigParser` + `RateLimitMiddleware`（`implements MiddlewareBase`，`onAgent` Semaphore 并发限流，超限 `AiBusinessException.tooManyRequests()`，非 Spring bean 由 factory 构造）+ `MiddlewareFactory`（按 type 构造，本期 `rate_limit`->`RateLimitMiddleware` params.maxConcurrent 默认 10；记账已由事件流 usage 聚合、OtelTracing 后续里程碑，均不归此）；`buildAgent` `middlewareFactory.build(middlewareConfig)` -> `builder.middlewares(list)`（与 `RagMiddleware` 并存，RAG 按 ragMode 装不归 middlewareConfig）。`AgentTemplate` 加 `toolGroups`/`middlewareConfig`（从 app 实时读，不入快照，不 override），`templateFromApp`/`Session` 读 `app.getToolGroups()`/`getMiddlewareConfig()`，`mergeOverride` 透传。`AiErrorCode.RATE_LIMITED(30905)` + `AiBusinessException.tooManyRequests()`。API 依据 sources jar 核实：`Toolkit.createToolGroup(name, desc, active)`/`updateToolGroups`/`ToolRegistration.tool().group(name).apply()`（`validateGroupExists` 要求组预先创建）/`MiddlewareBase` 5 default hook。**阶段 A-D 全部完成**。
