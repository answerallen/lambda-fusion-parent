...
- 单一事实来源（MUST）：同一份领域事实只能由一个组件解释或推导；下游消费其规范化输出，不得各自再维护一份平行的状态机或推导逻辑。按「持久性」区分归属：易失的运行时态（如调度器 trigger、在跑任务）归运行时组件解释；需跨重启一致的业务状态（如定时任务 `schedule_enabled` 启用态）归 DB 解释，运行时组件仅按其联动、不作为该状态的权威来源。
